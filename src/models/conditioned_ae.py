...
#log conditioning stats for debug
...
class ConditionedAE(nn.Module):
    def __init__(
        ...
        if is_embedding:
            #size of non_imaging and conditioning embedding size
            shape_features_size = embedding_size
            embedding_size = int(regression_mlp_expansion*256)
        ...
        if self.is_regression:
            ...
            if self.is_embedding:
                # encoder side
                self.FC1_e = nn.Sequential(nn.Linear(
                    in_features=256, out_features=embedding_size, bias=False))
                self.feature_regression_net = nn.Sequential(nn.ReLU(),
                                                            nn.Linear(in_features=embedding_size, out_features=shape_features_size, bias=False))
                # conditioning
                self.conditioning_net = nn.Sequential(nn.Linear(in_features=shape_features_size, out_features=shape_features_size, bias=False),
                                                      nn.ReLU(),
                                                      nn.Linear(in_features=shape_features_size, out_features=embedding_size, bias=False)
                                                      )
            else:
                self.feature_regression_net = nn.Sequential(nn.Linear(in_features=256, out_features=int(regression_mlp_expansion*256), bias=False),
                                                            nn.ReLU(),
                                                            nn.Linear(in_features=int(regression_mlp_expansion*256), out_features=embedding_size, bias=False))
    ...
    def forward(self, x=None, tab=None):
        ...
        #encoder
        # detach feature extraction and batch (N)orm + (A)ctivation to perform conditioning after skip connection
        if self.is_condition:
        ...
        if self.is_regression:
            encoder_features = self.gap(e5).view(x.shape[0],-1)
            #learn imaging feature embedding similar to conditional embedding 
            if self.is_embedding:
                imaging_embedding = self.FC1_e(encoder_features)
                #TRAIN
                if self.training:
                    y_feature_regression = self.feature_regression_net(imaging_embedding)
                    conditioning_embedding = self.conditioning_net(tab)
                ...
                #INFERENCE
                    ...
                    if self.is_inference_embedding:
                        ...
                        #shape features conditioning embedding
                        tab = conditioning_embedding
            else:
                y_feature_regression = self.feature_regression_net(encoder_features)
...
if __name__ == "__main__":
    ...
    model = ConditionedAE(spatial_dims=3,
                      in_channels=1,
                      out_channels=1,
                      act='relu',
                      norm='batch',
