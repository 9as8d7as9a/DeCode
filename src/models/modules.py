...
# _______________ TabAttention ____________________________________________________________________
# TabAttention based on: https://github.com/SanoScience/Tab-Attention/blob/main/src/models/tabattention.py
...
class INSIDE(TabularModule):
    def __init__(self,
        ...
        self.tanh_s = nn.Sigmoid()
        self.fc_s = nn.Linear(self.channel_dim // 2,
                              3 * self.channel_dim)  # TODO activity_regularizer=l2(config.INSIDE_PENELTY) (0.001)
    ...
    def attention(self, mu, sigma, shape, index):
        ...
        # Calculate relative coordinates. -> mu <-1;1>
    ...
    def forward(self, x, tab, reg=None):
        ...
        l2 = torch.norm(scale, p=2) + torch.norm(sigma_a, p=2) + torch.norm(sigma_b, p=2) + torch.norm(sigma_c, p=2)
    ...
    def hypernetwork(self, tab):
        ...
        s = self.tanh_s(self.fc_s(tab_emb))  # std
